...
# 1. do a pre-select to drop metadata etc
# 4. final select
...
wrangled_data <- combined_data %>%
  ...
  left_join(lookup_organisation, by = join_by(provider_site_code == organisation_code)) %>% 
  left_join(lookup_organisation, by = join_by(trust_code == organisation_code)) %>% 
  ...
  mutate(SiteName = case_when(provider_site_code == "RALC7" ~ "RFL - Chase Farm Hospital", # throws an error. cant use case_when()? need to do a calced column first i expect.
                              provider_site_code == "RAL01" ~ "RFL - Royal Free Hospital",
                              provider_site_code == "RALRA" ~ "RFL - Edgware Community Hospital",
                              provider_site_code == "RAL26" ~ "RFL - Barnet Hospital",
                              provider_site_code == "RALWH" ~ "RFL - Whittington Hospital",
                              provider_site_code == "F0U9Q" ~ "RFL - Finchley Memorial Hospital CDC",
                              trust_code == "RAL" ~ "RFL - Finchley Memorial Hospital CDC",
                              .default = organisation_name.x),
         ProviderTrust = case_when(provider_site_code == "F0U9Q" ~ "Royal Free London NHS Foundation Trust", # would be good to have this less hard coded. need to join parent org
                                   provider_site_code == "C4M4J" ~ "University College London Hospitals NHS Foundation Trust",
                                   provider_site_code == "P5B6O" ~ "Whittington Health NHS Trust",
                                   .default = organisation_name.y),
         # CancerPathwayFlag = case_when(priority_type_code == 3 ~ 1, # only using this for target logic. use provider submitted otherwise.
         #                               priority_type_code %in% c(2,1) ~ 0,
         #                               .default = NA)
         BreachScan = case_when(priority_type_code == 3 & TAT_scan > min(7*24, ncl_scan_target_hours) ~ 1,
                                TAT_scan > ncl_scan_target_hours ~ 1,
                                TAT_scan <= ncl_scan_target_hours ~0,
                                .default = NA),
         BreachScan7Day = case_when(priority_type_code == 3 & TAT_scan > min(5*24, ncl_scan_target_hours) ~ 1,
                                    TAT_scan > ncl_scan_target_hours ~ 1,
                                    TAT_scan <= ncl_scan_target_hours ~0,
                                    .default = NA),
         BreachReportNCL = case_when(priority_type_code == 3 & TAT_report > min(3*24, ncl_report_target_hours) ~ 1,
                                     TAT_report > ncl_report_target_hours ~ 1,
                                     TAT_report <= ncl_report_target_hours ~0,
                                     .default = NA),
         BreachReportNCLCancer7Day = case_when(priority_type_code == 3 & TAT_report > min(2*24, ncl_report_target_hours) ~ 1,
                                               TAT_report > ncl_report_target_hours ~ 1,
                                               TAT_report <= ncl_report_target_hours ~0,
                                               .default = NA),
         BreachReport4Week = case_when(TAT_report > 4*24*7 ~ 1,
                                       TAT_report <= 4*24*7 ~0,
                                       .default = NA),
         BreachReportNHSE = case_when(priority_type_code == 3 & TAT_report > min(3*24, nhse_report_tat_target_hours) ~ 1,
                                      TAT_report > nhse_report_tat_target_hours ~ 1,
                                      TAT_report <= nhse_report_tat_target_hours ~0,
                                      .default = NA),
         BreachReportNHSECancer7Day = case_when(priority_type_code == 3 & TAT_report > min(2*24, nhse_report_tat_target_hours) ~ 1,
                                                TAT_report > nhse_report_tat_target_hours ~ 1,
                                                TAT_report <= nhse_report_tat_target_hours ~0,
                                                .default = NA),
         BreachOverall = case_when(priority_type_code == 3 & TAT_overall > min(10*24, ncl_overall_tat_target_hours) ~ 1,
                                   TAT_overall > ncl_overall_tat_target_hours ~ 1,
                                   TAT_overall <= ncl_overall_tat_target_hours ~0,
                                   .default = NA),
         BreachOverallCancer7Day = case_when(priority_type_code == 3 & TAT_overall > min(7*24, ncl_overall_tat_target_hours) ~ 1,
                                             TAT_overall > ncl_overall_tat_target_hours ~ 1,
                                             TAT_overall <= ncl_overall_tat_target_hours ~0,
                                             .default = NA),
...
## Upload data to sandpit (basic bulk upload)
